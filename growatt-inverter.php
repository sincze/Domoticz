...
// $ mkdir pass2php
// $ cd pass2php
// $ wget https://github.com/sincze/Domoticz/growatt-inverter.php 
...
// In terminal execute 'php /home/pi/domoticz/scripts/pass2phps/growatt-inverter.php'
...
function retrieve_growatt_data($command)
{
	...
	if ($continue) {
		...
		if ($continue) {
			...
			$nowpower = $data['powerValue'];
			$todaypower = $data['todayStr'];

			$nowpower=filter_var($nowpower,FILTER_SANITIZE_NUMBER_INT)/10;
			$todaypower=filter_var($todaypower,FILTER_SANITIZE_NUMBER_INT);
			
			$str=( $nowpower.';'. $todaypower*100 );			
